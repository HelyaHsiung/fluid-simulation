...
class FluidSimulation:
    def __init__(self, m, n, windDirection=270.0, windLocationX=32, windLocationY=32, windSpeed=0.02, windNoise=10, windNoiseTimestep=300,
        ...
        self.ii = ii.ravel()
        self.jj = jj.ravel()
        self.Iij = self.fun_I(self.ii, self.jj)
        self.Iij0 = self.fun_I(self.ii + 1, self.jj)
        self.Iij1 = self.fun_I(self.ii - 1, self.jj)
        self.Iij2 = self.fun_I(self.ii, self.jj + 1)
        self.Iij3 = self.fun_I(self.ii, self.jj - 1)
        self.index = 0
    ...
    * Computed as: w = (del x U) where U is the velocity vector at (i, j).
    ...
    def curl(self, i, j):
        ...
        dvDx = (self.v[self.fun_I(i + 1, j)] - self.v[self.fun_I(i - 1, j)]) * 0.5
    ...
    def vorticityConfinement(self, vcX, vcY):
        ...
        for i in range(self.m):
            for j in range(self.n):
                ...
                dy = (self.curlData[self.fun_I(i, j + 1)] - self.curlData[self.fun_I(i, j - 1)]) * 0.5
    ...
    * and then trace it back in time to look for the 'particles' which end up at the cell's center.
    ...
    def advect(self, b, d, d0, u, v):
        ...
        t1 = y - j0
        t0 = 1 - t1
        Iij0 = self.fun_I(i0, j0)
        Iij1 = self.fun_I(i0, j1)
        Iij2 = self.fun_I(i1, j0)
        Iij3 = self.fun_I(i1, j1)
